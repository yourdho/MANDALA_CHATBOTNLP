...
export default function Create({ auth, price_schedules = [], master_addons = [] }) {
    ...
    return (
        <AuthenticatedLayout>
            <Head title="Admin | Tambah Fasilitas" />
            ...
            <div className="max-w-5xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
                <div className="mb-12">
                    ...
                    <h1 className="text-4xl md:text-6xl font-black italic uppercase tracking-tighter leading-none text-slate-900" style={{ color: 'var(--text-primary)' }}>
                        Tambah <span className="text-[#38BDF8]">Fasilitas Baru</span>
                    </h1>
                </div>
                    ...
                    <form onSubmit={submit} className="space-y-12">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                            ...
                            <div className="space-y-8">
                                ...
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black uppercase tracking-[0.3em] opacity-40 italic ml-4" style={{ color: 'var(--text-secondary)' }}>Foto Fasilitas (Banyak Sekaligus)</label>
                                    <div className="space-y-4">
                                        <div className="relative group/upload">
                                            ...
                                            <label htmlFor="image-upload" className="w-full border-2 border-dashed rounded-2xl p-10 flex flex-col items-center justify-center cursor-pointer hover:border-[#38BDF8] hover:bg-[#38BDF8]/5 transition-all text-center" style={{ borderColor: 'var(--border)' }}>
                                                <span className="text-3xl mb-4">📸</span>
                                                <p className="text-[10px] font-black uppercase tracking-widest" style={{ color: 'var(--text-primary)' }}>
                                                    + Tambah Foto/Video Fasilitas
                                                </p>
                                            </label>
                                        </div>

                                        {data.images.length > 0 && (
                                            <div className="grid grid-cols-4 gap-4 p-4 border-2 rounded-3xl" style={{ borderColor: 'var(--border)', background: 'var(--bg-base)' }}>
                                                {data.images.map((file, idx) => (
                                                    <div key={idx} className="relative aspect-square rounded-xl overflow-hidden border-2 border-[#38BDF8] group">
                                                        ...
                                                        }} className="absolute inset-0 bg-red-500/80 text-white font-black opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-[10px]">
                                                            DROP
                                                        </button>
                                                    </div>
                                                ))}
                                ...
                                <div className="space-y-3 max-h-[300px] overflow-y-auto no-scrollbar pr-2">
                                    {data.addons.map((addon, index) => (
                                        <div key={index} className="flex gap-3 items-center group">
                                            <input type="text" value={addon.name} onChange={e => updateAddon(index, 'name', e.target.value)} placeholder="Addon Name (e.g. Wasit)"
                        ...
                        <div className="flex items-center gap-4 ml-4">
                            <input type="checkbox" id="is_active" checked={data.is_active} onChange={e => setData('is_active', e.target.checked)}
                                className="w-6 h-6 rounded-lg text-[#38BDF8] focus:ring-[#38BDF8]/30 border-2 transition-all cursor-pointer shadow-sm"
                                style={{ background: 'var(--bg-base)', borderColor: 'var(--border)' }} />
                            <label htmlFor="is_active" className="text-xs font-black italic uppercase tracking-widest cursor-pointer" style={{ color: 'var(--text-primary)' }}>Aktifkan Fasilitas (Dapat Dipesan Umum)</label>
                        </div>

                        <div className="flex gap-4 pt-10 border-t" style={{ borderColor: 'var(--border)' }}>
                            <Link href={route('admin.facilities.index')} className="flex-1 py-5 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] text-center transition-all bg-slate-100 hover:bg-slate-200" style={{ background: 'var(--bg-base)', color: 'var(--text-secondary)' }}>
                                BATALKAN
                            </Link>
