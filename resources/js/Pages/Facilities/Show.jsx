...
export default function FacilityShow({ facility, timeSlots, user_vouchers = [] }) {
    ...
    const content = (
        <div className="min-h-screen selection:bg-[#38BDF8] selection:text-white transition-colors duration-300"
                ...
                <header className="fixed inset-x-0 top-0 z-50 flex items-center justify-between px-6 lg:px-20 h-24 border-b transition-all"
                    ...
                    <Link href="/" className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-[#38BDF8] rounded-xl flex items-center justify-center shadow-lg shadow-[#38BDF8]/30">
                            <span className="text-white font-black italic text-xl uppercase leading-none">M</span>
                        </div>
                    ...
                    <nav className="flex items-center gap-6">
                        <Link href="/" className="text-sm font-bold uppercase tracking-widest text-[#38BDF8] transition-colors">
                            Beranda
                        </Link>
                        <Link href={route('login')} className="px-6 py-2 bg-slate-100 text-slate-900 font-black rounded-full hover:bg-slate-900 hover:text-white transition-all text-xs uppercase tracking-widest shadow-lg">
                            Login
                        </Link>
                    </nav>
                </header>
            ...
            <main className={`max-w-7xl mx-auto px-6 lg:px-20 py-16 ${!auth?.user ? 'pt-40' : 'pt-10'}`}>
                    ...
                    <div className="lg:col-span-12 xl:col-span-8">
                        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                            ...
                            <div className="flex flex-wrap items-center gap-4">
                                <div className="flex items-center gap-2 px-4 py-2 rounded-xl shadow-sm border" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
                                    <span className="text-xs font-bold" style={{ color: 'var(--text-secondary)' }}>4.9 / 5.0 Rating</span>
                                </div>
                                <div className="flex items-center gap-2 px-4 py-2 rounded-xl shadow-sm border" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
                                    <span className="text-xs font-bold" style={{ color: 'var(--text-secondary)' }}>Mandala Arena Center</span>
                                </div>
                                <div className="flex items-center gap-2 px-4 py-2 rounded-xl shadow-sm border" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
                                    <span className="text-xs font-bold" style={{ color: 'var(--text-secondary)' }}>Online Payment Active</span>
                                </div>
                            </div>
                        </motion.div>
                    </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <label className="text-[10px] font-black uppercase tracking-[0.2em] italic" style={{ color: 'var(--text-secondary)' }}>Timeline Misi</label>
                                        <span className="text-[10px] font-bold text-[#38BDF8] uppercase bg-[#38BDF8]/5 px-3 py-1 rounded-full border border-[#38BDF8]/20">7-Day Window Active</span>
                                    </div>

                                    <div className="relative group/picker">
                                        <div id="date-scroll-track" className="flex gap-3 overflow-x-auto pb-4 no-scrollbar -mx-2 px-2 scroll-smooth snap-x snap-mandatory">
                                            {[...Array(21)].map((_, i) => {
                                                const d = new Date();
                                                ...
                                                return (
                                                    <button
                                                        key={dateString}
                                                        onClick={() => handleDateChange({ target: { value: dateString } })}
                                                        className={`flex-shrink-0 min-w-[75px] h-[95px] rounded-2xl flex flex-col items-center justify-center transition-all border-2 group relative overflow-hidden snap-center ${isActive
                                                            ? 'bg-slate-900 border-slate-900 text-white shadow-xl scale-105 z-10'
                                                            : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700 text-slate-500 hover:border-[#38BDF8]/50 active:scale-95'
                                                            }`}
                                                    >
                                                        {isActive && <div className="absolute top-0 inset-x-0 h-1.5 bg-[#38BDF8]" />}
                                                        <span className={`text-[9px] font-black uppercase tracking-widest mb-1 ${isActive ? 'text-[#38BDF8]' : 'text-slate-400'}`}>
                                                            {isToday ? 'Hari Ini' : isTomorrow ? 'Besok' : d.toLocaleDateString('id-ID', { weekday: 'short' })}
                                                        </span>
                                                        <span className="text-2xl font-['Permanent_Marker'] italic leading-none">{d.getDate()}</span>
                                                        <span className="text-[9px] font-bold uppercase opacity-60 mt-1">{d.toLocaleDateString('id-ID', { month: 'short' })}</span>
                                                    </button>
                                                );
                                            })}
                                        </div>
                                ...
                                {/* Info Harga / Jam */}
                                <div className="flex items-center justify-between py-6 border-y" style={{ borderColor: 'var(--border)' }}>
                                    <span className="text-xs font-bold uppercase tracking-widest" style={{ color: 'var(--text-secondary)' }}>Harga / Jam</span>
                                ...
                                {isMiniSoccer && selectedSlots.length > 0 && (
                                    <div className="flex items-start gap-4 p-4 rounded-2xl border" style={{ background: 'var(--bg-base)', borderColor: 'var(--border)' }}>
                                        <input
                                            type="checkbox"
                                            id="wasit"
                                            checked={withReferee}
                                            onChange={(e) => setWithReferee(e.target.checked)}
                                            className="mt-1 w-5 h-5 rounded text-[#FACC15] focus:ring-[#FACC15] border-slate-300"
                                        />
                                        <div>
                                            <label htmlFor="wasit" className="text-sm font-bold cursor-pointer block" style={{ color: 'var(--text-primary)' }}>Tambah Wasit Pertandingan</label>
                                            <p className="text-[10px] font-medium mt-1" style={{ color: 'var(--text-secondary)' }}>Biaya tambahan +Rp 50.000 (Flat) per sesi booking.</p>
                                        </div>
                                    </div>
                                )}
                                ...
                                >
                                    {selectedSlots.length > 0 ? (auth?.user ? 'Lanjut ke Pembayaran' : 'Booking Tanpa Akun') : 'Pilih Jam Terlebih Dahulu'}
                                </button>
                            </div>
                        </motion.div>
                    </div>
                    ...
                    <div className="lg:col-span-12 xl:col-span-8 space-y-12">
                        ...
                        <div className="rounded-[2.5rem] p-8 md:p-10 shadow-xl border overflow-hidden relative" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                                ...
                                <div className="flex items-center gap-4 px-4 py-2 rounded-full border" style={{ background: 'var(--bg-base)', borderColor: 'var(--border)' }}>
                                ...
                                {timeSlots.map(slot => (
                                    <button key={slot.time}
                                        ...
                                        className={`group relative rounded-2xl py-4 flex flex-col items-center justify-center transition-all border-2 ${selectedSlots.includes(slot.time)
                                            ...
                                            : slot.available
                                                ? 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 hover:border-[#38BDF8]/50 hover:text-[#38BDF8] cursor-pointer'
                                                : 'bg-slate-50 border-transparent text-slate-300 cursor-not-allowed overflow-hidden'
                                            }`}
                                    >
                        ...
                        <div className="rounded-[3rem] overflow-hidden border-2 relative group bg-slate-900 aspect-video shadow-2xl shadow-slate-200/50" style={{ borderColor: 'var(--border)' }}>
                        ...
                        {/* Description */}
                        <div className="p-10 rounded-[2.5rem] border shadow-xl" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
                            <h3 className="text-sm font-black uppercase tracking-widest mb-4" style={{ color: 'var(--text-secondary)' }}>Deskripsi Lapangan</h3>
                            <p className="text-lg leading-relaxed font-medium" style={{ color: 'var(--text-primary)' }}>
                                {facility.description || "Fasilitas Mandala Arena dirancang dengan mempertimbangkan performa dan kenyamanan."}
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        >
                            ...
                            >
                                <div className="bg-slate-50 p-8 border-b border-slate-100 flex items-center justify-between">
                                    ...
                                    <button onClick={() => !processing && setShowPaymentModal(false)} className="text-slate-400 hover:text-slate-900 transition-all font-bold">✕</button>
                                </div>
                                ...
                                <div className="p-8 space-y-8">
                                    <div className="bg-slate-900 rounded-2xl p-6 relative overflow-hidden text-white shadow-xl shadow-slate-900/20">
                                        ...
                                        <div className="flex flex-wrap gap-6 text-xs font-bold text-slate-300 uppercase tracking-widest">
                                            ...
                                            {withReferee && <span className="text-[#FACC15]">Wasit Aktif</span>}
                                        </div>
                                    ...
                                    <div className="space-y-4">
                                        {!auth?.user && (
                                            <>
                                                <input type="text" placeholder="Nama Lengkap" value={guestName} onChange={e => setGuestName(e.target.value)}
                                                    className="w-full rounded-xl bg-slate-50 border border-slate-200 text-slate-800 px-6 py-4 font-bold focus:ring-2 focus:ring-[#38BDF8] outline-none transition-all placeholder:text-slate-300" />
                                                <input type="email" placeholder="Email Aktif" value={guestEmail} onChange={e => setGuestEmail(e.target.value)}
                                                    className="w-full rounded-xl bg-slate-50 border border-slate-200 text-slate-800 px-6 py-4 font-bold focus:ring-2 focus:ring-[#38BDF8] outline-none transition-all placeholder:text-slate-300" />
                                            </>
                                        )}
                                        <input type="tel" placeholder="Nomor WhatsApp (Aktif)" value={guestPhone} onChange={e => setGuestPhone(e.target.value)}
                                            className="w-full rounded-xl bg-slate-50 border border-slate-200 text-slate-800 px-6 py-4 font-bold focus:ring-2 focus:ring-[#38BDF8] outline-none transition-all placeholder:text-slate-300" />
                                        {guestErrors.guest_phone && <p className="text-xs text-red-500 font-bold">{guestErrors.guest_phone}</p>}
                                    </div>

                                    <div className="grid grid-cols-1 gap-4">
                                        {PAYMENT_METHODS.map(m => (
                                            <button key={m.id} onClick={() => setPaymentMethod(m.id)}
                                                className={`p-5 rounded-2xl border-2 transition-all text-left flex items-start gap-4 ${paymentMethod === m.id ? 'border-[#38BDF8] bg-[#38BDF8]/5 shadow-md shadow-[#38BDF8]/10' : 'border-slate-100 bg-white hover:border-slate-300'}`}>
                                    ...
                                    <button onClick={confirmBooking} disabled={!paymentMethod || processing}
                                        className={`w-full rounded-2xl py-5 text-sm font-black transition-all shadow-lg uppercase tracking-widest ${paymentMethod && !processing ? 'bg-[#38BDF8] text-white hover:bg-[#38BDF8]/90 hover:-translate-y-1' : 'bg-slate-100 text-slate-400 cursor-not-allowed shadow-none'}`}>
    ...
    if (auth?.user) {
        return (
            <AuthenticatedLayout>
                <Head title={`${facility.name} | Mandala Arena`} />
                {content}
            </AuthenticatedLayout>
        );
    }
